...
```bash
/plugin marketplace update memory-store-plugins
```
...
Expected: Shows `memory-store@memory-store-plugins` as enabled
...
- [ ] Plugin installs via `/plugin install memory-store`
